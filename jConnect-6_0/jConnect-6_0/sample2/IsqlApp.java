...
public class IsqlApp
{
    ...
    static String _servicePrincipalName = null;
    static String _jaasConfFile = null;
        ...
        try
        {
            ...
            if (!processCommandline(args))
            {
                System.out.println(
                    ...
                    "\t\t[-v] [-I <input command file>] " +
                    "[-c <command terminator>] \n" + 
                    "\t\t[-s <starting tag/section marker>] " +
                    "[-e <ending tag/section marker>] \n" + 
                    "\t\t[-t <tag name> <new value>] " +
                    "[-n <section name to execute>] \n" + 
                    "\t\t[-N] [-K <service principal name for kerberos login>] \n" +
                    "\t\t[-F <path to JAAS login config file>] \n" +
                    "\t\t[-V <version {2,3,4,5,6}] -d\n");
                return (1);
            }
            ...
            if (_servicePrincipalName != null)
            {
               if (_jaasConfFile == null)
               {
                   System.out.println ("When using the kerberos login option " +
                       "[-K], you must also specify a JAAS login configuration " +
                       "file with the [-F] command-line option.");
                   return 1;
               }
               props.remove("user");
               props.remove("password");
               props.put("REQUEST_KERBEROS_SESSION", "true");
               props.put("SERVICE_PRINCIPAL_NAME", _servicePrincipalName);
               System.setProperty("javax.security.auth.useSubjectCredsOnly", "false");
               System.setProperty("java.security.auth.login.config", _jaasConfFile);
            }
                ...
                try 
                {
                    switch(arg.charAt(1))
                    {
                        ...
                        case 'K':
                            ...
                            if(_propValue != null)
                            {
                                _servicePrincipalName = _propValue;
                            }
                            else
                            {
                                errorCount++;
