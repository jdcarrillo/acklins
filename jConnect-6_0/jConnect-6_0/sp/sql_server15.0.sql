...
**      sp_jdbc_column_default        sybsystemprocs
...
declare @retval int
exec @retval = sp_version 'installjdbc',NULL,'jConnect (TM) for JDBC(TM)/6.0(Build 25570)/P/EBF12436/JDK14/Tue Feb 22  0:06:16 2005', 'start'
...
where d.ss_dtype = t.type and t.usertype *= e.user_type
     /* restrict results to 'real' datatypes, exclude float, date and time*/
      and t.name not in ('nchar','nvarchar','sysname','timestamp', 'float',
                         'datetimn','floatn','intn','moneyn', 'unichar',
                         'univarchar', 'daten', 'timen', 'date', 'time')
...
** types (listed)*/
      and (t.name in ('nchar','nvarchar', 'sysname')
...
if exists (select * from sysobjects where name = 'sp_jdbc_column_default')
begin
        drop procedure sp_jdbc_column_default
end
go
/** SECTION END: CLEANUP **/

create procedure sp_jdbc_column_default
/* Don't delete the following line. It is the checkpoint for sed */
/* Server dependent stored procedure add here ADDPOINT_COL_DEFAULT */
     (@obj_id int, @default_value varchar (16384) output)
as
...
go
exec sp_procxmode 'sp_jdbc_column_default', 'anymode'
go
grant execute on sp_jdbc_column_default to public
go
dump transaction sybsystemprocs with truncate_only 
go
...
/* Server dependent stored procedure add here ad ADDPOINT_COLS */
    ...
    declare @full_table_name  varchar(300)
    declare @table_id         int
    declare @char_bin_types   varchar(30)
    declare @uni_types        varchar (10)
    declare @column_default   varchar (16384)
    ...
    begin 
      ...
      fetch jdbc_columns_cursor1 into
        ...
        begin
          ...
          exec sp_jdbc_column_default @c_cdefault, @column_default out
...
insert spt_mda values ('GET_ISOLATION', 2, 'begin tran select @@isolation commit', 1, 7, 0, -1)
insert spt_mda values ('SET_ROWCOUNT', 2, 'set rowcount ?', 1, 7, 0, -1)
insert spt_mda values ('GET_AUTOCOMMIT', 2, 'begin tran select @@tranchained commit', 1, 7, 0, -1)
...
/* Server dependent stored procedure add here ad ADDPOINT_COLPRIVS */
    ...
    declare @curr_column        varchar (257)    
    declare @save_column_name varchar(257)
    declare @msg             varchar (255)
    declare @actual_table_name varchar(257)
...
@sp_name	varchar(255) = null	/* stored procedure name */
as
...
*/
declare @retval int
exec @retval = sp_version 'installjdbc',NULL,'jConnect (TM) for JDBC(TM)/6.0(Build 25570)/P/EBF12436/JDK14/Tue Feb 22  0:06:16 2005', 'end'
if (@retval != 0) select syb_quit()
go
